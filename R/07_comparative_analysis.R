...
counterfactual          <- load_output("eia_muni_coop_rate_comparison")
...
# Muni & Coop peer comparison (from script 04)
if (!is.null(counterfactual)) {
  latest_cf <- counterfactual %>% filter(year == max(year))
  summary_table <- summary_table %>%
    bind_rows(tibble(
      category = "Customer vs. shareholder",
      metric   = glue("Cumulative excess vs. Muni & Coop rates ({min(report_year_range)}–{max(report_year_range)})"),
      value    = dollar(latest_cf$cumulative_excess_b, accuracy = 0.1, suffix = "B"),
      note     = "Extra paid by GA Power customers vs. weighted-avg Muni & Coop rates; EIA Form 861"
    )) %>%
    bind_rows(tibble(
      category = "Customer vs. shareholder",
      metric   = glue("Per-customer annual excess vs. Muni & Coop rates ({max(report_year_range)})"),
      value    = dollar(latest_cf$annual_excess_per_customer, accuracy = 1),
      note     = "Additional cost per GA Power residential customer vs. Muni & Coop rates; EIA Form 861"
    )) %>%
    bind_rows(tibble(
      category = "Customer vs. shareholder",
      metric   = glue("Cumulative per-customer excess vs. Muni & Coop rates ({min(report_year_range)}–{max(report_year_range)})"),
      value    = dollar(latest_cf$cumulative_excess_per_customer, accuracy = 1),
      note     = "5-year total excess per GA Power residential customer vs. Muni & Coop rates; EIA Form 861"
    ))
}
